...
profiling_folder = "./profiled_dac_inputs/utkface_ResNet14_UTK/"
# npy output file path
output_name = "./adc_limits/utkface/dac_limits_ResNet14.npy"
...
layerNums = [0,1,2,3,5,6,8,9,10,12,13,17]
...
for k in range(1,len(layerNums)):
    ...
    if useGPU:
        ...
        DAC_max = ncp.array(ncp.max(x_dist_DAC))
        DAC_min = ncp.array(ncp.min(x_dist_DAC))
    ...
    if Nbits > 0:
        ...
        if useGPU:
            ...
            xmin_DAC = ncp.array(ncp.min(x_dist_DAC))
            xmax_DAC = ncp.array(ncp.percentile(x_dist_DAC,percentile_DAC))
    else:            
        xmin_DAC = DAC_min
        xmax_DAC = DAC_max
