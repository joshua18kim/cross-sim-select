...
# ideality/steepness error loop
ideality_vec = np.array([0.25, 0.425, 0.6, 0.775, 0.95, 1, 1.125, 1.3, 1.475, 1.65, 1.825, 2])

steepness = ideality_vec * 60 # units in mV/dec
...
accuracy_table = np.zeros((len(ideality_vec),config.Nruns,len(config.topk)))
...
for p in range(len(ideality_vec)):
        ...
        for j in range(Nlayers):
            ...
            if layerParams[j]['type'] in ('conv','dense'):
                ...
                params = set_params(task=config.task,
                    ...
                    select_ideality= ideality_vec[p], # config.select_ideality,
...
# Print results
for p in range(len(ideality_vec)):
    print("Steepness: {:.1f}       Accuracy: {:.3f}".format(steepness[p],accuracy_table[p][0][0]))
